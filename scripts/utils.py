...
def prepare_hd5_input(gene_enhancer_table, promoter_signals, gene_list, cells, num_features = 3, rna_method=None, rna_df_list=None):
    ...
    for gene in tqdm(gene_list):
        ...
        if rna_method is not None:
            ...
            PE_code, activity_list, distance_list, contact_list, gene_name, PE_links, gene_rna_df = encode_promoter_enhancer_links(gene_df, max_seq_len=2000, max_n_enhancer=60, max_distanceToTSS=100_000, add_flanking=False, rna_method=rna_method, rna_df_list=gene_rna_df_list)
            rna_signal_list.append(gene_rna_df)

        else:
            PE_code, activity_list, distance_list, contact_list, gene_name, PE_links = encode_promoter_enhancer_links(gene_df, max_seq_len=2000, max_n_enhancer=60, max_distanceToTSS=100_000, add_flanking=False, rna_method=rna_method, rna_df_list=gene_rna_df_list)
        contact_list = np.concatenate([[0], contact_list])
        distance_list = np.concatenate([[0], distance_list/1000])
        activity_list = np.concatenate([[0], activity_list])
        # activity_list = np.log10(0.1+activity_list)
